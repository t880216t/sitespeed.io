...
function getMedianMetric(metric) {
  return metric.median;
}
  ...
  for (let url of urls) {
    ...
    const metrics = {
      ...
      serverResponseTime: {
        ...
        metric: get(
          base.browsertime,
          'pageSummary.statistics.timings.pageTimings.serverResponseTime'
        ),
        f: time.ms
      },
      coachScore: {
      ...
      transferResponseCodes: {
        name: 'Response codes',
        metric: get(base.pagexray, 'pageSummary.responseCodes')
      },
      largestContentfulPaint: {
        name: 'largestContentfulPaint',
        metric: get(
          base.browsertime,
          'pageSummary.statistics.timings.largestContentfulPaint.renderTime'
        ),
        f: time.ms
      },
      firstContentfulPaint: {
        ...
        metric: get(
          base.browsertime,
          'pageSummary.statistics.timings.paintTiming["first-contentful-paint"]'
        ),
        f: time.ms
      },
      ttfb: {
        ...
        metric: get(
          base.browsertime,
          'pageSummary.statistics.timings.pageTimings.backEndTime'
        ),
        f: time.ms
      },
      inp: {
        ...
        metric: get(
          base.browsertime,
          'pageSummary.statistics.timings.interactionToNextPaint'
        ),
        f: time.ms
      }
      // googleWebVitals: {
      //   name: 'googleWebVitals',
      //   metric: get(base.browsertime, 'pageSummary.googleWebVitals')
      // }
    };
    ...
    for (const key of Object.keys(metrics)) {
      ...
      if (metric.metric !== undefined) {
        if (['Response codes', 'googleWebVitals'].includes(metric.name)) {
          fields.push({
            title: metric.name,
            value: metric.metric,
            short: true
          ...
          fields.push({
            title: metric.name,
            value: getMedianMetric(metric.metric),
            short: true
          });
        } else {
          fields.push({
    ...
    const attachement = {
      color,
      text,
      alias_info: get(base.browsertime, 'pageSummary.info.alias'),
      fields
    };
